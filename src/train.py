...
import wandb
def train_GAT(train_loader, test_loader, models, itemID_parties, optimizers, dataset,max_rating, phi,phi_inv, args):
        ...
        for epoch in tqdm(range(args.epochs)):
          ...
          for users, items, ratings in train_loader:
            ...
            for i in selected_parties:
              ...
              if this_a_grad is not None: 
                ...
                random_neg = torch.from_numpy(random_neg).float().to(args.device)
                random_pos = torch.from_numpy(random_pos).float().to(args.device)
                this_a_grad = (this_a_grad > random_pos) * args.clip + (this_a_grad < random_neg) * (-args.clip)
            ...
            for i in range(args.n_parties):
              ...
              # Convert a_grad to torch tensor
              # cor_ratio = torch.tensor(cor_ratio).to(args.device)
              if this_model.a.grad is not None:
                this_model.a.grad = torch.tensor(a_grad  * cor_ratio).to(args.device)
              for layer in range(args.n_layers):
          ...
          with torch.no_grad():
            ...
            wandb.log({"Train_RMSE": train_result, "Test_RMSE": test_result, "Loss": loss.item()})
...
def train_GGNN(train_loader, test_loader, models, itemID_parties, optimizers, dataset,max_rating, phi, phi_inv,  args):
  count, best_rmse = 0, 100
  ...
  for epoch in tqdm(range(args.epochs)):
    ...
    for users, items, ratings in train_loader:
      ...
      for i in selected_parties:
        ...
        phi_inv = phi_inv.to(args.device)
        this_useremb_aggs = torch.einsum('pu,huk->hpk', phi, this_useremb_aggs)
        prediction = this_model(this_users, this_items, this_useremb_aggs, phi_inv, cor_ratio)
        loss = this_model.sqr_loss(this_ratings, prediction)
        loss.backward()
    ...
    with torch.no_grad():
      ...
      print("Train_RMSE: {:.3f}, Test_RMSE: {:.3f}".format(
                train_result, test_result))
      wandb.log({"Train_RMSE": train_result, "Test_RMSE": test_result, "Loss": loss.item()}, step=epoch)
